...
function getDayName(date) {
  ...
  return arrOfDayNames[new Date(date).getDay()];
}
...
function getCountDaysInMonth(month, year) {
  const lastDayInMouthDate = new Date(year, month, 0);
  return lastDayInMouthDate.getDate();
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const numberOfDays = (new Date(dateEnd) - new Date(dateStart)) / msPerDay;
...
function isDateInPeriod(date, period) {
  ...
  const lowerLimit = new Date(period.start);
  const upperLimit = new Date(period.end);
